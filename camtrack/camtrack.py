...
def track_and_calc_colors(camera_parameters: CameraParameters,
    ...
    correspondences_i = build_correspondences(corner_storage[known_view_1[0]], corner_storage[known_view_2[0]])

    triangulation_parameters = TriangulationParameters(30, 0.1, 0.001)
    delta = 10

    points3d, corr_ids, _ = triangulate_correspondences(correspondences_i,
    ...
    tqdm_iterator = tqdm(zip(corner_storage, rgb_sequence), total=100, desc=descr_template.format('?', '?', '?'))
    for i, (corners_i, frame) in enumerate(tqdm_iterator):
        ...
        inliers = None
        try:
            retval, rvec, tvec, inliers = cv2.solvePnPRansac(point_cloud_builder.points[indexes_cloud],
